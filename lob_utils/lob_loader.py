...
class LOB_WF(Dataset):
    ...
    def __init__(self, h5_path='data/lob_raw.h5', split=1,
...
def get_wf_lob_loaders(h5_path='data/lob_raw.h5', window=50,
                       split=0, horizon=0, batch_size=128, class_resample=False):
    """
    ...
    train_dataset = LOB_WF(h5_path=h5_path, split=split, train=True, n_window=window, normalization='std',
                           epsilon=1e-15, horizon=horizon)

    test_dataset = LOB_WF(h5_path=h5_path, split=split, train=False, n_window=window, normalization='std',
                          epsilon=1e-15, horizon=horizon)
